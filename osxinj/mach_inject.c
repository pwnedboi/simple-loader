...
mach_error_t mach_inject(const mach_inject_entry threadEntry, const void* paramBlock, size_t paramSize, pid_t targetProcess, vm_size_t stackSize )
{
    ...
    if( !err && paramBlock != NULL && paramSize )
    {
        ...
        if( !err )
        {
            ...
            err = vm_write( remoteTask, remoteParamBlock, (pointer_t) paramBlock, (mach_msg_size_t)paramSize );
        }
    }
...
#define STACK_CONTENTS_SIZE ((1+PARAM_COUNT) * sizeof(unsigned int))
        ...
        // push stackContents
        err = vm_write( remoteTask, remoteStack, (pointer_t) stackContents, STACK_CONTENTS_SIZE);
...
void* fixedUpImageFromImage ( const void* image, unsigned long imageSize, unsigned int jumpTableOffset, unsigned int jumpTableSize, ptrdiff_t fixUpOffset)
{
    ...
    if (*(unsigned char *) jumpTable == 0xff)
    {
        ...
        for (entry = 0; entry < jumpTableCount; entry++)
        {
            ...
            fprintf(stderr, "at %p correcting %p to %p\n", (char *)jumpTable -2, jmpValue, jmpValue - fixUpOffset);
